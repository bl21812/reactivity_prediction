...
class RNAInputDataset(Dataset):
    ...
    def __init__(self, df, pretrain=False, seq_length=512, device=None, watson_crick=False, wc_dir=None):
        '''
    ...
    def get_corresponding_wc_matrix(self, sequence_id: str):
        filename = next(self.wc_dir.rglob(f"{sequence_id}.txt"))
        wc_matrix = np.zeros((self.seq_length, self.seq_length))
        ...
        return torch.tensor(wc_matrix).to(self.device) if self.device != "cpu" else torch.tensor(wc_matrix)
    ...
    def __getitem__(self, idx):
        ...
        if self.watson_crick:
            return inp, label, pad_mask, self.get_corresponding_wc_matrix(self.df['sequence_id'].iloc[idx])
        else:
